...
exports.play = {
    ...
    process: function(client, msg, suffix, isEdit) {
        if (isEdit) return;
        let arr = msg.guild.channels
            .filter((v) => v.type == 'voice')
            .filter((v) => v.members.has(msg.author.id));
        // Make sure the user is in a voice channel.
        if (arr.length == 0) {
            return msg.channel.send( wrap('You\'re not in a voice channel.'));
        }
        ...
        msg.channel.send( wrap('Searching...')).then((response) => {
            ...
            youtubeDL.getInfo(suffix, ['-q', '--no-warnings', '--force-ipv4'],
                (err, info) => {
                // Verify the info.
                    if (err
                    ...
                    // Queue the video.
                    response.edit( wrap('Queued: ' + info.title))
                        .then((resp) => {
                            queue.push(info);
...
exports.skip = {
    ...
    process: function(client, msg, suffix) {
        ...
        if (voiceConnection === null) {
            return msg.channel.send( wrap('No music being played.'));
        }
...
*/
exports.dequeue = {
    description: 'Dequeues the given song index from the song queue. '
        + ' Use the queue command to get the list of songs in the queue.',
    process: function(client, msg, suffix) {
        // Define a usage string to print out on errors
        const usageString = 'The format is "!dequeue <index>".'
            + '  Use !queue to find the indices of each song in the queue.';
        ...
        if (!suffix) {
            return msg.channel.send(
                wrap(
                    'You need to specify an index to remove from the queue. '
                    + usageString
                )
            );
        }
        ...
        // Get the arguments
        let split = suffix.split(/(\s+)/);

        // Make sure there's only 1 index
        if (split.length > 1) {
            return msg.channel.send(
                wrap('There are too many arguments.  ' + usageString)
            );
        }
        ...
        if (!isNaN(index)) {
            ...
            if (index >= 0 && index < queue.length) {
                ...
                return msg.channel.send(
                    wrap('The index is out of range.  ' + usageString)
                );
            }
        } else {
            return msg.channel.send(
                wrap('That index isn\'t a number.  ' + usageString)
            );
        }
...
exports.volume = {
    ...
    description: 'set music playback volume as a fraction, '
        + 'a percent, or in dB',
    process: function(client, msg, suffix) {
        // Get the voice connection.
        const voiceConnection = client.voiceConnections.get(msg.guild.id);
        ...
        if (voiceConnection.player.dispatcher) {
            if (suffix == '') {
                let displayVolume =
                    Math.pow(voiceConnection.player.dispatcher.volume,
                        0.6020600085251697) * 100.0;
                msg.channel.send(wrap('volume: ' + displayVolume + '%'));
            } else {
                if (suffix.toLowerCase().indexOf('db') == -1) {
                    if (suffix.indexOf('%') == -1) {
                        if (suffix > 1) suffix /= 100.0;
                        voiceConnection.player.dispatcher
                            .setVolumeLogarithmic(suffix);
                    } else {
                        let num = suffix.split('%')[0];
                        voiceConnection.player.dispatcher
                            .setVolumeLogarithmic(num/100.0);
                    }
                } else {
                    let value = suffix.toLowerCase().split('db')[0];
...
/**
 * Getting the author voice channel
 * @param {string} msg the message
 * @return {string} voice channel of the author
 */
function getAuthorVoiceChannel(msg) {
    let voiceChannelArray = msg.guild.channels.filter((v)=>v.type == 'voice')
        .filter((v) =>
            v.members.has(msg.author.id)
        ).array();
